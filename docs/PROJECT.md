...
当前代码已经实现 v0.2 产品闭环：`forestfix serve` 提供任务控制台，Provider 适配 Hermes / Codex / Claude，确定性验证器负责所有硬门槛，SQLite 保存任务与候选证据，已通过候选只能由用户显式应用到独立本地分支。远程 webhook、PR 创建与能力库仍按后续阶段实施。
